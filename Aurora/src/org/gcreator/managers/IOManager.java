...
public class IOManager {
    ...
    public static String getPreferredTypeFor(GFile f) throws IOException {
        ...
        throw new IOException("Unknown preferred type for '" + f.type + "', "
                + f.type.getClass() + "Object: " + o + " (owner " + f.getClass().getName() + ")");
    }
    ...
    public static String getSimpleNameFor(Folder f) throws IOException {
        ...
        if (c == ActionGroup.class) {
            return "ActionGroup";
        }
        if (c == ActorGroup.class) {
            return "ActorGroup";
        }
        if (c == GCLGroup.class) {
            return "CodeGroup";
        }
        if (c == ImageGroup.class) {
            return "ImageGroup";
        }
        if (c == PathGroup.class) {
            return "PathGroup";
        }
        if (c == SceneGroup.class) {
            return "SceneGroup";
        }
        if (c == SnippetGroup.class) {
            return "SnippetGroup";
        }
        if (c == SoundGroup.class) {
            return "SoundGroup";
        }
        if (c == SpriteGroup.class) {
            return "SpriteGroup";
        }
        if (c == StaticGroup.class) {
            return "StaticGroup";
        }
        if (c == TilesetGroup.class) {
            return "TilesetGroup";
        }
        if (c == TimelineGroup.class) {
            return "TimelineGroup";
        }
        throw new IOException("Unknown simple type for" + f.getClass().getName());
    }
