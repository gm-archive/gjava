 ...
 * 
 * 
 * This class is needed because com.golden.gamedev.engine.audio.WaveRenderer does
 ...
 */
public class GWaveRenderer extends WaveRenderer implements LineListener {
    ...
    private static int rendererStatus = GWaveRenderer.UNINITIALIZED;
     ...
     */
    public GWaveRenderer() {
        if (GWaveRenderer.rendererStatus == GWaveRenderer.UNINITIALIZED) {
            GWaveRenderer.rendererStatus = GWaveRenderer.INITIALIZING;

            Thread thread = new Thread() {

                public final void run() {
                    try {
                        URL sample = GWaveRenderer.class.getResource("Sample.wav");
                        ...
                        GWaveRenderer.volumeSupported = clip.isControlSupported(FloatControl.Type.VOLUME) || clip.isControlSupported(FloatControl.Type.MASTER_GAIN);
                        ...
                        for (int i = 0; i < mixers.length; i++) {
                            if ("Java Sound Audio Engine".equals(mixers[i].getName())) {
                                GWaveRenderer.mixer = AudioSystem.getMixer(mixers[i]);
                            }
                        }

                        GWaveRenderer.available = true;
                    } catch (Throwable e) {
                        System.err.println("WARNING: Wave audio playback is not available!");
                        GWaveRenderer.available = false;
                    }

                    GWaveRenderer.rendererStatus = GWaveRenderer.INITIALIZED;
                }
            };
    ...
    public boolean isAvailable() {
        if (GWaveRenderer.rendererStatus != GWaveRenderer.INITIALIZED) {
            int i = 0;
            while (GWaveRenderer.rendererStatus != GWaveRenderer.INITIALIZED && i++ < 50) {
                try {
                    Thread.sleep(50L);
        ...
        return GWaveRenderer.available;
    }
    ...
    protected void playSound(InputStream in) {
        try {
            ...
            DataLine.Info info = new DataLine.Info(Clip.class, ain.getFormat(),
                    ((int) ain.getFrameLength() * format.getFrameSize()));
            // workaround for Java 1.5
            if (GWaveRenderer.mixer != null) {
                this.clip = (Clip) GWaveRenderer.mixer.getLine(info);
    ...
    public boolean isVolumeSupported() {
        return GWaveRenderer.volumeSupported;
    }
