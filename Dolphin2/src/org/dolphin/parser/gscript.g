...
array returns [String value]
  : valuee=(WORD|OIVAR|GLOBALVAR) '[' (e=expression{$value=$e.value;})? (',' e1=expression{$value = $e.value + ","+$e1.value;})? ']'  {$value = pc.array($valuee.text,$value);} ('.' vvv=variable {$value+="."+pc.variable($vvv.value);})?
;
